...
|---|---|---|---|
| **Contamination rate** | Share of interactions classified as purchased for another person, by category and month | None exists publicly | Produce first public estimate with confidence intervals |
...
| Seasonality check | Nov–Dec rate materially above annual mean | **Stop and revise** — detector is not capturing gift intent |
...
|---|---|---|
| **Annotator LLM** | Qwen3.5-9B (primary), Gemma 4 12B (secondary) | Apache 2.0; runs quantised on consumer GPU; customer text never leaves the environment; reproducible by third parties, unlike commercial APIs |
...
| **Distillation target** | ModernBERT-base | Native 8,192-token context prevents truncating the review region where recipient disclosure typically appears; state-of-the-art encoder efficiency |
...
**Resource fit.** Total cost is $0. The annotation job runs overnight on a consumer GPU or Kaggle's free tier. The distillation step, which is the design decision that makes corpus-scale labelling affordable, is documented in the literature: annotating a 6.2-million-document corpus directly with a frontier model was estimated at ~$8,990, against ~$15 for a 1,000-document sample.
...
- **Seasonality.** The detector reads text and has no access to the calendar. If its output nonetheless peaks in November–December (and secondarily in February and May), that is external evidence of validity obtained free. *Caveat stated in advance: review timestamps lag purchase timestamps, so peaks will be displaced; the existence of the peak matters, not its exact position.*
- **Category face validity.** Toys > Video Games > Grocery.
...
flowchart TD
    ...
    C --> D["LLM Detector<br/>Qwen3.5-9B via vLLM<br/>guided JSON decoding"]
